CLASS z2ui5_tool_cl_app_03 DEFINITION PUBLIC.
...
CLASS Z2UI5_TOOL_CL_APP_03 IMPLEMENTATION.
  ...
  METHOD init_table_output.
    ...
    DATA(lt_cols)   = z2ui5_tool_cl_utility=>get_fieldlist_by_table( mr_table->* ).
...
*        DELETE ms_view-t_tab WHERE selkz = abap_true.
      ...
      WHEN 'BUTTON_CUSTOM'.
        client->message_box_display( `custom action called` ).
      ...
      WHEN `POPUP_LAYOUT_LOAD`.
        ...
        z2ui5_tool_cl_utility=>trans_xml_2_object(
          EXPORTING
            xml  = ls_layout2-data
          IMPORTING
             data = ms_layout
      ...
      WHEN `BUTTON_SAVE_LAYOUT`.
        DATA(ls_layout) = VALUE ty_s_db_layout(
          data = z2ui5_tool_cl_utility=>trans_data_2_xml( ms_layout )
          name = mv_layout_name
          ).
        INSERT ls_layout INTO TABLE mt_db_layout.

      WHEN 'BUTTON_SEARCH'.
*        client->cursor_set( value #(
*            cursorpos = '99'
*            id = 'SEARCH'
*            selectionend = '99'
*            selectionstart = '99'
...
*    app-next-path = app-next-path && `(` && ms_detail-uuid && `)`.
    ...
    DATA(page) = view2->object_page_layout(
    ...
    sections->object_page_section( titleuppercase = abap_false id = 'goalsSectionSS1' title = '2014 Goals Plan'
                     ...
                     "   editable = mv_check_editable
                        value = client->_bind( ms_detail-data ) ).
  ...
  METHOD z2ui5_on_render_main.
    ...
    DATA(view) = z2ui5_cl_xml_view=>factory( client ).
    DATA(view2) = view->page( id = `page_main`
            title          = 'abap2UI5 - List Report'
            ...
            shownavbutton  = abap_true
                ...
                )->input( value = client->_bind_edit( mv_name )
                )->button( press = client->_event( 'LOAD' ) text = 'Load'
        )->get_parent( )->get_parent( ).
    ...
    IF mr_table IS NOT INITIAL.
      ...
      IF mr_table IS NOT INITIAL.
        ...
        DATA(lv_lines) = CONV string( lines(  <tab> ) ).
      ENDIF.

      tab->header_toolbar(
            )->toolbar(
                )->title( text = ms_layout-title && ` (` && shift_right( lv_lines ) && `)` level = `H2`
                    )->toolbar_spacer(
                )->button(
...
*                  press = client->_event( 'BUTTON_CUSTOM' )
  ...
  METHOD z2ui5_on_render_pop_detail.
    ...
    app-view_popup = lo_popup->get_root( )->xml_get( ).
...
*                 )->get_parent( )->get_parent( ).
    ...
    app-view_popup = ro_popup->get_root( )->xml_get( ).
...
*    .
  ...
  METHOD z2ui5_set_download_csv.
    ...
    i_view->zz_plain( `<html:iframe src="data:text/csv;base64,` && LV_base && `" hidden="hidden" />`).
